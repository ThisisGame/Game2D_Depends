...
FI_BOOL testStreamMultiPageSave(const char *input, const char *output, int input_flag, int output_flag) {
	...
	if(src) {
		...
		if (file != NULL) {
			...
			FI_BOOL bSuccess = FreeImage_SaveMultiBitmapToHandle(fif, src, &io, (fi_handle)file, output_flag);
			assert(bSuccess);
	...
	if (src_file != NULL) {
		...
		if(src) {
			...
			if (dst_file != NULL) {
				...
				FI_BOOL bSuccess = FreeImage_SaveMultiBitmapToHandle(fif, src, &io, (fi_handle)dst_file, output_flag);
				assert(bSuccess);
