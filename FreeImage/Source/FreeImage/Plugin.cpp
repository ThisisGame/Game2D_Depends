...
void DLL_CALLCONV
FreeImage_Initialise(FI_BOOL load_local_plugins_only) {
...
void * DLL_CALLCONV
FreeImage_Open(PluginNode *node, FreeImageIO *io, fi_handle handle, FI_BOOL open_for_reading) {
	...
	if ((fif >= 0) && (fif < FreeImage_GetFIFCount())) {
		...
		if (node) {
			if(node->m_plugin->save_proc != NULL) {
				...
				FI_BOOL result = node->m_plugin->save_proc(io, dib, handle, -1, flags, data);
...
FreeImage_Save(FREE_IMAGE_FORMAT fif, FIBITMAP *dib, const char *filename, int flags) {
	...
	if (handle) {
		FI_BOOL success = FreeImage_SaveToHandle(fif, dib, &io, (fi_handle)handle, flags);
...
int DLL_CALLCONV
FreeImage_SetPluginEnabled(FREE_IMAGE_FORMAT fif, FI_BOOL enable) {
	if (s_plugins != NULL) {
		PluginNode *node = s_plugins->FindNodeFromFIF(fif);

		if (node != NULL) {
			FI_BOOL previous_state = node->m_enabled;
