...
public :
	CacheFile(const std::string filename, FI_BOOL keep_in_memory);
	~CacheFile();

	FI_BOOL open();
	void close();
	FI_BOOL readFile(BYTE *data, int nr, int size);
...
private :
	...
	FI_BOOL unlockBlock(int nr);
	FI_BOOL deleteBlock(int nr);
